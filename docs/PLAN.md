...
- **Acceptance Criteria**:
  - 지원 기호: 표눈(K), 코(P), 감침(S), 2목 일반편(K2tog), 2목 왼쪽편(ssk) 등 (대바늘 기본 10개)
...
- **Acceptance Criteria**:
  ...
  - 점대칭(회전 대칭) 모드: 중심점 기준 180° 회전 대칭 — 숄 칼라, 레이스, 원형 요크 등에 활용
  - 영역 선택 & 복사/붙여넣기: 직사각형 영역을 드래그로 선택하고 Ctrl+C / Ctrl+V로 복사·붙여넣기
  - 패턴 타일 반복: 선택 영역을 가로(열 방향) 또는 세로(행 방향)으로 N회 자동 타일링
  - 단 반복 블록 마커: 반복할 행(row) 범위를 지정하고 "N회 반복" 레이블 부여 — PDF 출력 시 반복 구간 자동 표기
  - 형태선(Shape Guide) 오버레이: 목선·진동·소매산 등 의류 실루엣을 격자 위에 반투명 가이드라인으로 표시
  - 반복 구간 설정: 특정 영역을 드래그하여 '반복'으로 지정하면 도안 전체에 해당 무늬 복사
  - 칸 크기 조정 (픽셀 단위): 10px ~ 40px (width 기준, height는 비율에 따라 자동 계산)
  - 셀 종횡비 옵션: 4:5 기본(뜨개 코 비율) / 1:1 정사각형 선택 가능
  - 간격 조정 (기선/격자)
...
- **User Story**: "사용자로서 나는 완성된 도안을 PDF로 내보내서 인쇄할 수 있고 싶다"
- **Acceptance Criteria**:
  - PDF 파일 생성 (A4, Letter 크기 지원)
  - 도안의 기호 차트 포함
  - 도안 정보 헤더 포함 (제목, 난이도, 실 종류, 이름 등)
...
- **User Story**: "사용자로서 나는 대바늘 또는 코바늘 도안을 선택해서 각각의 기호를 사용하고 싶다"
- **Acceptance Criteria**:
  - 대바늘(Knitting) / 코바늘(Crochet) 선택 옵션
  - 각 종류별 표준 기호 세트 제공
  - 선택에 따라 기호 팔레트 동적 변경
  - 기존 도안의 종류 변경 가능 (경고 메시지)
...
- 차트마인더 https://www.chart-minder.com/demo
...
- **PDF 라이브러리**: jsPDF vs react-pdf vs html2pdf
- **상태관리**: Zustand vs Redux vs Context (선택: Zustand 간단함)
- **테스트**: Jest vs Vitest
- **배포**: Vercel vs AWS vs Docker

### 디자인 결정

- **기호 색상**: 10개 기호 → 10개 색상? (가독성 vs 심미성)
- **격자 스타일**: 선 vs 점 (경험 있는 뜨개인 선호도)
- **초보자 모드**: 단순화된 UI vs 풀 기능 UI
